...
export const PairSelectButton = (props) => {
  const { pairs, selectedIndex, setSelectedIndex, tokenInfo } = props;
  const [show, setShow] = useState(false);
  return (
    <div className="w-max relative">
      <button
        ...
        data-dropdown-toggle="dropdownHover"
        data-dropdown-trigger="hover"
        className="w-full h-5 flex gap-1 space-grotesk-500 text-xxs text-white bg-[#1c2147] hover:ring-2 hover:ring-blue-300 rounded-md p-2 text-center items-center justify-between"
        type="button"
        onClick={() => setShow((p) => !p)}
      >
        {pairs.length == 0 ? (
          ...
          <div className="w-full flex items-center overflow-hidden overflow-ellipsis text-nowrap">
            {pairs[selectedIndex]?.baseToken?.symbol + '/' + pairs[selectedIndex]?.quoteToken?.symbol}
          </div>
        )}
        <img src="/assets/icon/ic_arrow_down.svg" width={6} alt="down-logo" />
      </button>
      ...
      <div
        id="dropdownHover"
        className={`w-max z-10 ${show ? "flex" : "hidden"
          } flex-col absolute space-grotesk-500 text-xxs overflow-hidden text-white bg-[#3A3D42] opacity-100 rounded-md text-center items-center justify-between border border-solid border-white/20`}
        onBlur={() => setShow(false)}
      >
        {pairs?.map((_v, _i) => {
          return (
            <div
              key={_v.pairAddress}
              className="flex gap-2 px-2 w-full hover:bg-slate-500 items-center cursor-pointer "
              onClick={() => {
                setSelectedIndex(_i);
                setShow(false);
              }}
            >
              {pairs[_i]?.baseToken?.symbol + '/' + pairs[_i]?.quoteToken?.symbol}
            </div>
          );
        })}
      </div>
