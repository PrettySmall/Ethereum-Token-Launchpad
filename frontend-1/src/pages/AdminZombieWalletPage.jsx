...
export default function AdminZombieWalletPage({ className }) {
  ...
  useEffect(() => {
    ...
    if (projects && projects.length > 0) {
      setActiveProjects(projects.filter((project) => project.zombies[0] != ""));
    } else {
      setActiveProjects([])
    }
  }, [projects]);
  ...
  useEffect(() => {
    ...
    if (activeProjects && activeProjects.length > 0) {
      ...
      getProjectsEthBalance(activeProjects);
    } else {
      setWalletAllChecked(false);
      setWalletChecked([]);
      setEthBalances([]);
  ...
  const handleWalletAllChecked = () => {
    ...
    if (newWalletAllChecked) {
      let temp = {}
      ethBalances.map((v, index) => {
        if (temp[activeProjects[index].zombies[0]]) {
          return;
        } else {
          temp[activeProjects[index].zombies[0]] = 1;
          sum = sum + Number(v)
        }
      });
    }
  ...
  const handleCollectAllEth = async () => {
    ...
    try {
      ...
      for (let i = 0; i < activeProjects.length; i++) {
        if (walletChecked[i]) {
          wallets = [...wallets, activeProjects[i].zombies[0]];
        }
      }
              ...
              {activeProjects.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <td className="text-left">{item.zombies[0]}</td>
                    <td
