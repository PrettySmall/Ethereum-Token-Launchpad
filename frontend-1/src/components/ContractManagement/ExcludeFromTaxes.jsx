...
import { isValidAddress, updateMaxTxnExemption, updateTaxExemption } from "../../utils/methods";
...
export default function ExcludeFromTaxes() {
    ...
    const handleUpdateTaxExemption = async () => {
        if (!isConnected) {
        ...
        setLoadingPrompt("Updating Tax Exemption...");
        setOpenLoading(true);

        const ret = await updateTaxExemption(signer, template, tokenAddress, walletAddress, isExcluded)
        if (ret) {
            toast.success("Tax Exemption Updated!");
        } else {
            toast.warn("Failed to update tax exemption!");
        }
        ...
        <div className="w-full flex flex-col gap-3">
            ...
            <div className="relative flex h-full text-white bg-transparent justify-evenly bg-clip-border">
                <button
                    ...
                    onClick={handleUpdateTaxExemption}
                >
